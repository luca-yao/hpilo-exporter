...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def watch_processor(self):
        ...
        if processors_values is not None:
            for cpu_idx, cpu in processors_values.items():
                cpu_label_parts = cpu.get('label', '').split()
                cpu_label = cpu_label_parts[1] if len(cpu_label_parts) > 1 else ''
                cpu_name = cpu.get('name', '').strip()
                cpu_status = cpu.get('status', '')
                cpu_speed = cpu.get('speed', '')
                if cpu_name:
                   self.gauges["processor_detail"].labels(product_name=self.product_name, server_name=self.server_name, cpu_id=cpu_label, name=cpu_name, status=cpu_status, speed=cpu_speed).set(1 if "OK" in cpu_status else 0)

    def watch_memory(self):
        memory_values = self.embedded_health.get('memory', {})
#        if memory_values is not None:
        if memory_values is not None and 'memory_details_summary' in memory_values:
            for cpu_idx, cpu in memory_values['memory_details_summary'].items():
                total_memory_size = 0 if (cpu.get('total_memory_size') == 'N/A') else int(cpu['total_memory_size'].split()[0])
                self.gauges["memory_detail"].labels(product_name=self.product_name, server_name=self.server_name, cpu_id=cpu_idx.split("_")[1], operating_frequency=cpu.get('operating_frequency', ''), operating_voltage=cpu.get('operating_voltage', '')).set(total_memory_size)
    def watch_fan(self):
...
#                capacity_w = 0 if p_value.get("capacity") == "N/A" else int(p_value.get("capacity").split()[0])
                capacity_w = 0 if (p_value is None or p_value.get("capacity", "N/A") == "N/A") else int(p_value.get("capacity").split()[0])
    ...
    def do_GET(self):  # noqa: C901
        ...
        if url.path == self.server.endpoint and ilo_host and ilo_user and ilo_password and ilo_port:
            ...
            try:
                ...
                print_err("Error while getting Product Name:", e)
            ...
            if ilo.get_fw_version()["management_processor"] == 'iLO3':
                ...
                # get nic information
                nic_information = self.embedded_health.get('nic_information', [])
                for nic in nic_information:
                    if not isinstance(nic, dict):
                        print_err(f"Unexpected nic information: {nic}")
                        continue
                    try:
                        value = ['OK', 'Disabled', 'Unknown', 'Link Down'].index(nic.get('status', 'Unknow'))
                    except ValueError:
                        value = 4
                        print_err('unrecognised nic status: {}'.format(nic.get('status', 'Unknow')))
                    ...
                    self.gauges['nic_status'].labels(product_name=self.product_name, server_name=self.server_name,
                                                     nic_name=nic_name, ip_address=ip_address).set(value)
